{   // AXIS
    ...
    function axis3 (size, nameList, back) {
        const V3 = BABYLON.Vector3
        const anchorSize = 0.05 * size

        const axisX = BABYLON.Mesh.CreateLines(nameList[0], [
            back ? new V3(-size, 0, 0) : V3.Zero(),
            new V3(size, 0, 0),
            new V3(size * 0.95, anchorSize, 0),
            new V3(size, 0, 0),
            new V3(size * 0.95, -anchorSize, 0)
        ], scene)
        axisX.color = new BABYLON.Color3(1, 0, 0)
        const axisY = BABYLON.Mesh.CreateLines(nameList[1], [
            back ? new V3(0, -size, 0) : V3.Zero(),
            new V3(0, size, 0),
            new V3(-anchorSize, size * 0.95, 0),
            new V3(0, size, 0),
            new V3(anchorSize, size * 0.95, 0)
        ], scene)
        axisY.color = new BABYLON.Color3(0, 1, 0)
        const axisZ = BABYLON.Mesh.CreateLines(nameList[2], [
            back ? new V3(0, 0, -size) : V3.Zero(),
            new V3(0, 0, size),
            new V3(0, -anchorSize, size * 0.95),
            new V3(0, 0, size),
            new V3(0, anchorSize, size * 0.95)
        ], scene)
